...
LOOKBACK_PERIODS = int(os.getenv("LOOKBACK_PERIODS", "20"))  # Need at least a few periods for EMA calculation
LEVERAGE = int(os.getenv("LEVERAGE", "3"))

# EMA parameters
SMMA_PERIOD = int(os.getenv("SMMA_PERIOD", "10"))
...
# Log file
LOG_FILE = os.getenv("LOG_FILE", "smma_slope_strategy_v5_log.txt")
...
last_order_time = 0
smma_value = 0
smma_slope = 0  # Slope of the EMA
previous_smma_slope = 0  # Previous slope direction for detecting changes
...
def calculate_smma_slope():
    """Calculate EMA and its slope"""
    global candle_data, smma_value, smma_slope, previous_smma_slope, slope_direction_changed
    
    if candle_data is None or len(candle_data) < SMMA_PERIOD + SLOPE_LOOKBACK:
        log_message(f"Not enough data to calculate SMMA. Need at least {SMMA_PERIOD + SLOPE_LOOKBACK} periods.")
        return
    ...
    try:
        ...
        df = candle_data.copy()
        # Calculate SMMA
        smma = df['close'].rolling(window=SMMA_PERIOD, min_periods=SMMA_PERIOD).mean()
        
        # Calculate SMMA for the rest of the values
        for i in range(SMMA_PERIOD, len(df)):
            smma.iloc[i] = (smma.iloc[i-1] * (SMMA_PERIOD-1) + df['close'].iloc[i]) / SMMA_PERIOD
        
        df['smma'] = smma
        
        # Get the current EMA value
        smma_value = df['smma'].iloc[-1]
        
        # Store previous slope direction
        previous_slope_direction = 1 if previous_smma_slope > 0 else -1 if previous_smma_slope < 0 else 0
        
        # Calculate slope (current EMA - EMA from SLOPE_LOOKBACK periods ago)
        current_ema = df['smma'].iloc[-1]
        previous_ema = df['smma'].iloc[-1-SLOPE_LOOKBACK]
        smma_slope = (current_ema - previous_ema) / previous_ema  # Percentage change
        
        # Get current slope direction
        current_slope_direction = 1 if smma_slope > 0 else -1 if smma_slope < 0 else 0
        ...
        if previous_slope_direction != 0 and current_slope_direction != 0:
            if previous_slope_direction != current_slope_direction:
                ...
                log_message(f"SMMA SLOPE DIRECTION CHANGED: {previous_slope_direction} -> {current_slope_direction}")
            else:
                slope_direction_changed = False
        
        # Update previous slope for next calculation
        previous_smma_slope = smma_slope
        
        if smma_slope > 0:
            log_message(f"10-period SMMA: {smma_value:.4f}, Slope: POSITIVE ({smma_slope*100:.4f}%)")
        else:
            log_message(f"10-period SMMA: {smma_value:.4f}, Slope: NEGATIVE ({smma_slope*100:.4f}%)")
        
        return smma_value, smma_slope
    
    except Exception as e:
        log_message(f"Error calculating EMA: {e}")
        return None, None
...
def update_candle_data(trade_data):
    ...
    try:
        ...
        if trade_dt.minute != latest_timestamp.minute or trade_dt.hour != latest_timestamp.hour:
            ...
            # Recalculate EMA when a new candle is created
            calculate_smma_slope()
        else:
...
def place_aggressive_orders(api):
    """Place aggressive orders based on SMMA slope"""
    ...
    try:
        ...
        if smma_slope > 0:
            ...
            order_side = "buy"
            log_message("Placing aggressive BUY orders due to positive SMMA slope")
        else:
...
def order_management_thread(api):
    ...
    try:
        while True:
            ...
            # Place/refresh orders based on SMMA slope
...
if __name__ == "__main__":
    ...
    with open(LOG_FILE, "w") as f:
        f.write(f"=== SMMA SLOPE STRATEGY V5 LOG - {datetime.datetime.now()} ===\n\n")
